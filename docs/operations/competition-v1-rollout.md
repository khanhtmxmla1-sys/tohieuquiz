...
1. Apply migrations through `0079_competition_runtime_rollout.sql` and verify the migration registry. The runtime `competition_v1` flag must remain disabled during backend deployment.
...
Database rollback is a separate, explicitly approved operation. Rehearsed rollback scripts live under `workers/migrations/rollback` and must run in reverse order from 0079 to 0069. Migration `0079` is rolled back by disabling its audited runtime flag rather than deleting rollout history. Migration `0078` contains the immutable result-correction ledger, so dropping it destroys audit evidence. Do not run destructive schema rollback as part of an automatic application rollback.
