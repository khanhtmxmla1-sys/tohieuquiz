...
export const getResultAchievementStats = (db: D1Database, username: string) =>
    db.prepare(`
        SELECT
            ...
            COALESCE(SUM(CASE WHEN CAST(strftime('%H', datetime(r.submitted_at, '+7 hours')) AS INTEGER) < 7 THEN 1 ELSE 0 END), 0) AS early_bird_count,
            COALESCE(SUM(CASE WHEN CAST(strftime('%H', datetime(r.submitted_at, '+7 hours')) AS INTEGER) >= 21 THEN 1 ELSE 0 END), 0) AS night_owl_count,
