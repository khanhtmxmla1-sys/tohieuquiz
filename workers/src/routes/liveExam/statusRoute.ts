...
export const handleStatusRoute: LiveExamRouteHandler = async (context) => {
  ...
  try {
    ...
    if (!currentParticipant) return errorResponse('Forbidden: Join session first', 403);
    ...
    const effectiveEndsAt = LiveExamService.getEffectiveParticipantEndsAt(
      session.endsAt,
      currentParticipant.individual_ends_at,
    );
    const timeRemaining = ['active', 'paused'].includes(session.status) && effectiveEndsAt
      ? calculateTimeRemaining(effectiveEndsAt, session.status === 'paused' ? session.pausedAt : undefined)
      : undefined;
    return jsonResponse({
      success: true,
      session: {
      ...
      participantSubmittedAt: currentParticipant.submitted_at || undefined,
      timeRemaining,
    });
  } catch (error: unknown) {
