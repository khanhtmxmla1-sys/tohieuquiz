...
export const upsertCatalogItemMock = async (
    ...
    if (normalized.id) {
        ...
        const updated: GiftCatalogItem = {
            ...
            stockRemaining: Math.min(normalized.stockTotal, Math.max(0, state.catalog[index].stockRemaining + normalized.stockTotal - state.catalog[index].stockTotal)),
            stockTotal: normalized.stockTotal,
            lowStockThreshold: normalized.lowStockThreshold,
            weeklyLimitPerStudent: normalized.weeklyLimitPerStudent,
    ...
    const created: GiftCatalogItem = {
        ...
        isActive: normalized.isActive ?? true,
        stockTotal: normalized.stockTotal,
        stockRemaining: normalized.stockTotal,
        lowStockThreshold: normalized.lowStockThreshold,
        weeklyLimitPerStudent: normalized.weeklyLimitPerStudent,
        scopeType: normalized.scopeType,
        schoolId: normalized.schoolId,
