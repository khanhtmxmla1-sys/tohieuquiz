...
export const deliverOrderMock = async (orderId: string, actor: GiftOrderActor): Promise<GiftOrder> => {
    const state = readMockState();
    const order = getOrderById(state, orderId);
    assertCanManageOrder(order, actor);
    if (order.status !== 'APPROVED') throw new Error('Đơn hàng không ở trạng thái có thể trao quà.');
    ...
    pushEvent(state, { type: 'ORDER_DELIVERED', orderId, studentId: order.studentId, actor: actor.username });
    saveMockState(state);
    return order;
};
