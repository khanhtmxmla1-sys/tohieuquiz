...
import {
  competitionArticleTemplateTypes,
  competitionArticleTypeLabel,
  createCompetitionArticleTemplate,
} from './competitionArticleTemplates';
...
const isLegacyGuideArticle = (article: StaffCompetitionArticleDto): boolean => (
...
const CompetitionArticleManager: React.FC<CompetitionArticleManagerProps> = ({ campaignId, isAdmin, campaign, rounds = [] }) => {
  ...
  const fillNewArticleTemplate = () => {
    ...
    setError(null);
    setMessage('Đã điền mẫu từ cấu hình. Kiểm tra nội dung trước khi tạo bản nháp.');
  };
    ...
    <article className="rounded-xl border border-slate-200 bg-slate-50 p-4" aria-labelledby="competition-article-manager-title">
          ...
          return (
            <div key={article.id} className="rounded-lg border border-slate-200 bg-white p-3">
              <div className="flex flex-wrap items-start justify-between gap-2">
                <div><p className="font-semibold text-slate-900">{article.title}</p><p className="text-xs text-slate-500">{competitionArticleTypeLabel(article.type)} · {article.status} · {shortId}</p></div>
              </div>
      ...
      {isAdmin && (
        <form className="mt-4 space-y-3 rounded-lg border border-dashed border-slate-300 bg-white p-3" onSubmit={createArticle}>
          ...
          {existingArticleOfSelectedType && (
            <p role="status" className="rounded-lg bg-amber-50 p-3 text-sm text-amber-800">
              Đã có bài viết loại {competitionArticleTypeLabel(newArticle.type)} ({existingArticleOfSelectedType.status}). Kiểm tra bài hiện có trước khi tạo để tránh trùng nội dung.
            </p>
          )}
          {canFillTemplate && (
            <div className="flex flex-wrap items-center gap-2">
              <button type="button" onClick={fillNewArticleTemplate} disabled={Boolean(pending)} className="min-h-11 rounded-lg border border-indigo-300 bg-white px-4 py-2 text-sm font-semibold text-indigo-700 disabled:opacity-50">Điền mẫu từ cấu hình</button>
              <span className="text-xs text-slate-500">Chỉ điền dữ liệu đã cấu hình; chưa gọi API.</span>
            </div>
          )}
