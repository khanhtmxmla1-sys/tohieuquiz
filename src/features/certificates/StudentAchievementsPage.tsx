...
const StudentAchievementsPage: React.FC<StudentAchievementsPageProps> = () => {
    const { certificates, isLoading, isLoadingMore, error, refetch, loadMore, hasMore, total } = useCertificates();
                    ...
                    <div className="flex items-center gap-3">
                        ...
                        <div>
                            ...
                            <p className="text-xs text-gray-500">
                                {isLoading
                                    ? 'Đang tải...'
                                    : (hasMore || total !== certificates.length
                                        ? `${certificates.length}/${total} chứng nhận`
                                        : `${total} chứng nhận`)}
                            </p>
                        </div>
                    </div>
            ...
            <div className="max-w-4xl mx-auto px-4 pt-6">
                ...
                {!isLoading && !error && certificates.length === 0 && (
                    <div className="flex flex-col items-center justify-center py-20 text-center">
                        <div className="mb-6"><ModuleIcon name="achievements" size="xl" /></div>
                    ...
                    <>
                        <div className="mb-6 flex items-center gap-3">
                            <div className="flex-1 bg-white rounded-2xl shadow-sm border border-amber-100 p-4 flex items-center gap-3">
                                ...
                                <div>
                                    <p className="text-2xl font-black text-amber-500">{total}</p>
                        ...
                        {hasMore && (
                            <div className="mt-6 flex justify-center">
                                <button
                                    ...
                                    className="rounded-xl border border-indigo-200 bg-white px-5 py-2.5 text-sm font-semibold text-indigo-600 hover:bg-indigo-50 disabled:opacity-50"
                                >
                                    {isLoadingMore ? 'Đang tải thêm...' : 'Xem thêm chứng nhận'}
                                </button>
                            </div>
                        )}
                    </>
