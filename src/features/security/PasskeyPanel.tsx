...
const formatDate = (value: string | null): string => {
  if (!value) return 'Ch?a s? d?ng';
  const date = new Date(value);
  return Number.isNaN(date.getTime()) ? 'Kh?ng x?c ??nh' : date.toLocaleString('vi-VN');
};

export const PasskeyPanel: React.FC = () => {
  const [passkeys, setPasskeys] = useState<AccountPasskey[]>([]);
  const [label, setLabel] = useState('Thi?t b? c?a t?i');
  ...
  const load = useCallback(async () => {
    ...
    try {
      ...
      showError(error instanceof Error ? error.message : 'Kh?ng th? t?i passkey.');
    } finally {
      setLoading(false);
    }
  }, []);
  ...
  const add = async () => {
    ...
    try {
      ...
      showSuccess('?? th?m passkey. M?t kh?u v?n c? th? d?ng ?? kh?i ph?c.');
    } catch (error) {
      showError(error instanceof Error ? error.message : 'Kh?ng th? th?m passkey.');
    } finally {
      setBusyId(null);
    }
  };
  ...
  const remove = async (passkey: AccountPasskey) => {
    ...
    try {
      ...
      showSuccess('?? x?a passkey.');
    } catch (error) {
      showError(error instanceof Error ? error.message : 'Kh?ng th? x?a passkey.');
    } finally {
      setBusyId(null);
    }
  };
  ...
  return (
    <div className="mt-6 border-t pt-5" aria-labelledby="passkey-settings-title">
      <div className="flex items-start gap-3">
        ...
        <div>
          <h4 id="passkey-settings-title" className="font-bold text-slate-900">Passkey cho gi?o vi?n</h4>
          <p className="mt-1 text-sm text-slate-500">??ng nh?p b?ng kh?a b?o m?t ho?c sinh tr?c h?c. Passkey b? sung m?t kh?u, kh?ng thay th? ph??ng ?n kh?i ph?c.</p>
        </div>
      </div>

      {!supported && (
        <div className="mt-3 rounded-xl bg-amber-50 p-3 text-sm text-amber-800">Tr?nh duy?t ho?c thi?t b? n?y ch?a h? tr? WebAuthn.</div>
      )}

      <div className="mt-4 flex flex-col gap-2 sm:flex-row">
        <label className="flex-1 text-sm font-semibold text-slate-700">T?n thi?t b?
        ...
        <button type="button" onClick={() => void add()} disabled={!supported || busyId !== null} className="mt-auto inline-flex min-h-11 items-center justify-center gap-2 rounded-xl bg-blue-600 px-4 font-bold text-white disabled:opacity-50">
          {busyId === 'new' ? <Loader2 className="h-4 w-4 animate-spin" /> : <Plus className="h-4 w-4" />} Th?m passkey
        </button>
      </div>

      <div className="mt-4 space-y-2" role="list" aria-label="Danh s?ch passkey">
        {loading ? (
          <div className="flex min-h-20 items-center justify-center"><Loader2 className="h-5 w-5 animate-spin" /></div>
        ) : passkeys.length === 0 ? (
          <p className="rounded-xl bg-slate-50 p-4 text-sm text-slate-500">Ch?a c? passkey.</p>
          ...
          <article key={passkey.id} role="listitem" className="flex flex-wrap items-center justify-between gap-3 rounded-xl border p-4">
            <div className="flex items-start gap-3">
              ...
              <div>
                ...
                <div className="mt-1 text-xs text-slate-500">{passkey.deviceType} ? {passkey.backedUp ? '?? sao l?u' : 'Ch? thi?t b? n?y'}</div>
                <div className="text-xs text-slate-400">D?ng g?n nh?t: {formatDate(passkey.lastUsedAt)}</div>
              </div>
            </div>
            <button type="button" onClick={() => void remove(passkey)} disabled={busyId !== null} className="inline-flex min-h-10 items-center gap-2 rounded-xl border border-red-200 px-3 text-sm font-semibold text-red-700 disabled:opacity-50">
              {busyId === passkey.id ? <Loader2 className="h-4 w-4 animate-spin" /> : <Trash2 className="h-4 w-4" />} X?a
            </button>
          </article>
        ))}
      </div>
