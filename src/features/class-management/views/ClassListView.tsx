...
import { Button, DataFreshnessNotice, Skeleton } from '../../../components/common';
...
export const ClassListView: React.FC<ClassListViewProps> = ({
    classes, isAdmin, onSelectClass, onCreateClick, onTransferClick, onDeleteClick, isLoading, error, onRetry, isOnline, lastUpdatedAt,
}) => {
    ...
    if (isLoading && classes.length === 0) {
        return <Skeleton label="Đang tải lớp học" className="min-h-[320px] w-full rounded-2xl" />;
    }
    ...
    return (
        <div className="space-y-6">
            <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
                ...
                {isAdmin && <Button onClick={onCreateClick} variant="primary" icon={<Plus className="w-5 h-5" />} disabled={!isOnline} title={!isOnline ? 'Cần kết nối mạng để tạo lớp.' : undefined}>Tạo lớp mới</Button>}
            </div>

            {error && <div role="alert" className="bg-red-50 border border-red-200 rounded-xl p-4 flex flex-col sm:flex-row sm:items-center justify-between gap-3 text-red-700"><span className="inline-flex items-center gap-2"><AlertCircle className="w-5 h-5" />{error}</span><Button variant="secondary" onClick={onRetry} icon={<RefreshCw className="w-4 h-4" />} disabled={!isOnline} title={!isOnline ? 'Cần kết nối mạng để thử lại.' : undefined}>Thử lại</Button></div>}

            <DataFreshnessNotice staleAt={lastUpdatedAt} isOffline={!isOnline} isRefreshing={isLoading && classes.length > 0} />
            ...
            {!isLoading && !error && classes.length === 0 ? (
                <div className="bg-white rounded-2xl border border-gray-100 p-12 text-center">
                    ...
                    {isAdmin ? <><p className="text-gray-500 mb-6">Hãy tạo lớp học đầu tiên của bạn</p><Button onClick={onCreateClick} variant="primary" disabled={!isOnline}>Tạo lớp ngay</Button></> : <p className="text-gray-500 mb-6">Giáo viên chưa được cấp lớp. Vui lòng liên hệ quản trị viên.</p>}
                </div>
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                    {visibleClasses.map((classroom) => <ClassCard key={classroom.id} classroom={classroom} isAdmin={isAdmin} onClick={() => onSelectClass(classroom)} onTransfer={() => onTransferClick(classroom)} onDelete={() => onDeleteClick(classroom)} serverActionsDisabled={!isOnline} />)}
                </div>
            )}
        </div>
    );
