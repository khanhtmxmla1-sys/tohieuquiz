...
const LoginForm: React.FC<LoginFormProps> = ({
    ...
    const roleButtonClass = (role: 'student' | 'teacher') => (
        `flex min-h-11 flex-1 items-center justify-center gap-2 rounded-[13px] px-3 text-sm font-semibold transition-[background-color,color,box-shadow,transform] duration-300 ease-[cubic-bezier(0.16,1,0.3,1)] focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-[#2563eb] focus-visible:ring-offset-2 active:scale-[0.98] ${
            activeTab === role
                ? 'border border-[#dbeafe] bg-white text-[#1d4ed8] shadow-none'
                : 'border border-transparent text-[#64748b] hover:bg-white/70 hover:text-[#1e3a8a]'
        }`
    );
    ...
    return (
        <section className="login-page-reveal order-1 mx-auto w-full max-w-[460px] lg:order-2 lg:mx-0 lg:justify-self-start" aria-labelledby="login-title">
            <div className="rounded-[30px] border border-[#dce5f1] bg-white/50 p-1 shadow-[0_28px_70px_-46px_rgba(30,58,138,0.42)]">
                <div className="rounded-[26px] bg-white px-4 py-5 sm:px-7 sm:py-7 lg:px-8">
                    <header className="mb-5 sm:mb-6">
                        ...
                        <p className="mt-2 text-sm leading-6 text-[#64748b] sm:text-[0.95rem]">
                            Chọn vai trò và đăng nhập để tiếp tục sử dụng TôHiệuQuiz.
                        </p>
                    </header>

                    <div
                        className="mb-5 flex rounded-[17px] border border-[#e3eaf4] bg-[#f3f7fd] p-1 sm:mb-6"
