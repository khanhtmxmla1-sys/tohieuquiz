...
const RecentSubmissionsPanel: React.FC<RecentSubmissionsPanelProps> = ({
    ...
    <section aria-labelledby="recent-submissions-heading" className="rounded-[14px] border border-slate-200 bg-white p-4 sm:p-5">
        <div className="flex items-start justify-between gap-3">
            <div>
                <p className="text-sm font-medium text-sky-700">Hoạt động hôm nay</p>
                <h2 id="recent-submissions-heading" className="mt-1 text-xl font-semibold tracking-tight text-slate-900">
                    Bài vừa nộp
                </h2>
            </div>
            <button
                type="button"
                onClick={onViewAll}
                className="inline-flex min-h-10 items-center gap-1.5 rounded-[10px] px-2.5 text-xs font-medium text-sky-700 transition-colors hover:bg-sky-50 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-sky-700"
            >
                Xem tất cả
        ...
        {isLoading ? (
                ...
                {submissions.map((result) => {
                    ...
                    return (
                        <li key={result.id} className="flex items-start gap-3 py-3 first:pt-1 last:pb-0">
                            <span className="flex size-10 shrink-0 items-center justify-center rounded-full border border-sky-200 bg-sky-50 text-sm font-semibold text-sky-700">
                                {result.studentName.trim().charAt(0).toUpperCase() || '?'}
                            </span>
                            <div className="min-w-0 flex-1">
                                <div className="flex items-start justify-between gap-3">
                                    <div className="min-w-0">
                                        <p className="truncate text-sm font-semibold text-slate-900">{result.studentName}</p>
                                        <p className="mt-0.5 line-clamp-1 text-xs text-slate-600">
                                            <span>vừa nộp</span> {result.quizTitle || 'Bài kiểm tra'}
                                        </p>
                                    </div>
                                    <span className={`shrink-0 text-sm font-semibold ${isPass ? 'text-emerald-700' : 'text-amber-700'}`}>
                                        {score} điểm
                                    </span>
                                </div>
                                <div className="mt-2 flex flex-wrap items-center gap-3">
                                    <span className={`inline-flex items-center gap-1 text-[11px] font-medium ${isPass ? 'text-emerald-700' : 'text-amber-700'}`}>
                                        {isPass
            ...
            <div className="py-10 text-center">
                <Inbox aria-hidden="true" className="mx-auto size-8 text-slate-400" />
