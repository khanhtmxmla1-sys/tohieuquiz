...
const loadAuditTools = async () => {
  ...
  try {
    ...
    const scoringModule = await server.ssrLoadModule('/src/domain/quiz-scoring/index.ts');
    return {
...
export const auditQuestionRows = async (inputRows) => {
  const rows = parseJsonRows(inputRows);
  const { mapLiveExamQuestionRow, normalizeQuestionForGrading } = await loadAuditTools();
  const findings = [];
